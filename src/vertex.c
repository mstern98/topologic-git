...
struct vertex *create_vertex(struct graph *graph, void (*f)(struct graph *, struct vertex_result *), int id, void *glbl)
{
	if (!graph || !f)
		return NULL;
...
int modify_vertex(struct vertex *vertex, void (*f)(struct graph *, struct vertex_result *), void *glbl)
{
	if (!vertex)
		return -1;
